  ...
  async function render(el) {
    ...
    let html = `
      <div class="card">
        ...
        ${items.map((item) => `
          <div class="list-row">
            ${item.image ? `<img class="item-thumb" src="${item.image}" alt="${Utils.escapeHtml(item.name)}">` : `<span class="item-thumb item-thumb-placeholder">${(typeof Icons !== "undefined" ? Icons.get("cafe", 18) : "")}</span>`}
