  ...
  async function settleSession(deviceId) {
    ...
    App.openModal(`
      ...
      <div class="form-group"><label>روش پرداخت</label>
        <select id="settlePayType" onchange="Consoles.toggleCombinedPayment('settlePayType', 'combinedFieldsSession', ${total})"><option value="wallet">کیف‌پول</option><option value="debt">بدهکاری</option><option value="cash">نقدی</option><option value="card">کارتی</option><option value="combined">ترکیبی (نقدی + کارتی)</option></select>
      </div>
      <div id="combinedFieldsSession" style="display:none; margin-top:8px;">
        <div class="form-group"><label>مبلغ کارتی</label><input type="number" id="combinedCardAmountSession" min="0" oninput="Consoles.updateCombinedCheck('combinedCardAmountSession', 'combinedCashAmountSession', 'combinedCheckSession', ${total})"></div>
        <div class="form-group"><label>مبلغ نقدی</label><input type="number" id="combinedCashAmountSession" min="0" oninput="Consoles.updateCombinedCheck('combinedCardAmountSession', 'combinedCashAmountSession', 'combinedCheckSession', ${total})"></div>
